...
// @downloadURL  https://github.com/itribs/tampermonkeyJs/raw/master/sycm-item-source-30day-table.user.js
...
(function() {
    ...
    $(function(){
        $(document).on('click', '.source-detail-trend', function() {
            ...
            let pageName = getParams($('.item-source-dialog-download').attr('href'))['pageName'];
            ...
            parent.find('.ant-modal-title div span').eq(0).text('流量趋势 - ' + pageName)
            flowItemTableLink.text(tableShowing ? '显示图表': '显示表格');
        })
    })
})();
