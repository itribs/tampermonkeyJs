...
(function () {
    ...
    let excludeTags = ['汽车测评', '美妆', '萌宠', '音乐', '旅行', '汽车', '科技数码', '家居家装', '影视娱乐', '财经投资', '美妆教程', '妆容展示', '护肤保养', '美妆测评种草', '日常宠物', '特别宠物', '宠物周边', '歌曲演唱', '乐器演奏', '音乐教学', '音乐其他', '真人出镜', '游戏实况', '游戏剧情', '游戏解说', '游戏资讯', '汽车测评', '汽车知识', '汽车周边', '美妆测评', '3C数码测评', '汽车测评', '3C数码', '家居电器', '家装设计', '装修知识', '影视娱乐', '传统金融', '互联网金融', '财经知识'];
...
`;
        ...
        while (true) {
            ...
            for (let i = 0; i < userDatas.data.data.authors.length; i++) {
                ...
                for (let key in user.tags_relation) {
                    for (let tagIndex = 0; tagIndex < user.tags_relation[key].length; tagIndex++) {
                        if (excludeTags.indexOf(user.tags_relation[key][tagIndex]) > -1) {
                            continue outerFor;
                        }
                    }
                }
    ...
    async function getUserData (user) {
        ...
        if (!distributionDatas.data.data) {
            return null;
        }
